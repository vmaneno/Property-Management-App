...
module.exports = async (req, res) => {
  ...
  try {
    ...
    await client.query('DELETE FROM prop_units');
    await client.query('DELETE FROM properties');
    await client.query('DELETE FROM clients');
    await client.query('DELETE FROM agencies');
    await client.query('DELETE FROM reset_tokens');
    ...
    for (const a of agencies) {
      const hash = await bcrypt.hash(a.Password, ROUNDS);
      const data = { ...a, Password: '' };
      await client.query(
        `INSERT INTO agencies (agency_code,username,password_hash,is_master,active,data) VALUES ($1,$2,$3,$4,$5,$6)`,
        [a.AgencyCode, a.UserName, hash, a.Master==='Y', a.Active==='Y', JSON.stringify(data)]
      );
    }
    ...
    for (const c of clients) {
      ...
      const data = { ...c, Password: '' };
      await client.query(
        `INSERT INTO clients (client_code,agency_code,username,password_hash,active,data) VALUES ($1,$2,$3,$4,$5,$6)`,
        [c.ClientCode, c.AgencyCode, c.UserName, hash, c.Active==='Y', JSON.stringify(data)]
      );
    }
    ...
    for (const p of properties) {
      await client.query(
        `INSERT INTO properties (property_code,client_code,agency_code,active,data) VALUES ($1,$2,$3,$4,$5)`,
        [p.PropertyCode, p.ClientCode, p.AgencyCode, p.Active==='Y', JSON.stringify(p)]
      );
    }
    ...
    for (const u of units) {
      await client.query(
        `INSERT INTO prop_units (unit_code,property_code,active,data) VALUES ($1,$2,$3,$4)`,
        [u.UnitCode, u.PropertyCode, u.Active==='Y', JSON.stringify(u)]
      );
    }
    ...
    for (const t of tenants) {
      const hash = await bcrypt.hash(t.Password, ROUNDS);
      const data = { ...t, Password: '' };
      await client.query(
        `INSERT INTO tenants (tenant_id,property_code,unit_code,username,password_hash,active,data) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [t.TenantID, t.PropertyCode, t.UnitCode||null, t.UserName||null, hash, t.Active==='Y', JSON.stringify(data)]
      );
    }
