...
async function seed() {
  ...
  try {
    ...
    for (const a of agencies) {
      ...
      const { Password, UserName, Active, Master, ...rest } = a;
      await client.query(
        `INSERT INTO agencies (agency_code, username, password_hash, is_master, active, data)
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [a.AgencyCode, UserName, hash, Master === 'Y', Active === 'Y',
         JSON.stringify({ ...rest, UserName, Password: '', Active, Master })]
      );
    }
    ...
    for (const c of clients) {
      const hash = c.Password ? await bcrypt.hash(c.Password, ROUNDS) : null;
      const { Password, UserName, Active, ...rest } = c;
      await client.query(
        `INSERT INTO clients (client_code, agency_code, username, password_hash, active, data)
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [c.ClientCode, c.AgencyCode, UserName || null, hash, Active === 'Y',
         JSON.stringify({ ...rest, UserName: UserName || '', Password: '', Active })]
      );
    }
    ...
    for (const p of properties) {
      ...
      await client.query(
        `INSERT INTO properties (property_code, client_code, agency_code, active, data)
         VALUES ($1,$2,$3,$4,$5)`,
        [p.PropertyCode, p.ClientCode, p.AgencyCode, Active === 'Y', JSON.stringify({ ...rest, Active })]
      );
    }
    ...
    for (const u of units) {
      ...
      await client.query(
        `INSERT INTO prop_units (unit_code, property_code, active, data)
         VALUES ($1,$2,$3,$4)`,
        [u.UnitCode, u.PropertyCode, Active === 'Y', JSON.stringify({ ...rest, Active })]
      );
    }
    ...
    for (const t of tenants) {
      const hash = t.Password ? await bcrypt.hash(t.Password, ROUNDS) : null;
      const { Password, UserName, Active, ...rest } = t;
      await client.query(
        `INSERT INTO tenants (tenant_id, property_code, unit_code, username, password_hash, active, data)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [t.TenantID, t.PropertyCode, t.UnitCode || null, UserName || null, hash, Active === 'Y',
         JSON.stringify({ ...rest, UserName: UserName || '', Password: '', Active })]
      );
    }
